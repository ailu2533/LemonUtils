...
extension Date {
    ...
    static var weekdayIndex: [Int] {
        ...
        return (0..<7).map { (firstDay + $0 - 1) % 7 + 1 }
    }
...
//     }
    ...
    var calendarDisplayDays: [Date] {
        ...
        // 添加当前月的天数
        for dayOffset in 0 ..< numberOfDaysInMonth {
            let newDay = calendar.date(byAdding: .day, value: dayOffset, to: startOfMonth!)!
            days.append(newDay)
        }

        // 用下个月的日期填充剩余的格子，直到总数达到42（6行 * 7列）
        while days.count < 42 {
            if let lastDay = days.last, let nextDay = calendar.date(byAdding: .day, value: 1, to: lastDay) {
                days.append(nextDay)
            }
        }
